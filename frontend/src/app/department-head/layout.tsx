...
const navigation: NavItem[] = [
  ...
  {
    label: "Lesson Plans",
    href: "/department-head/lesson-plans",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: "Evaluations",
    href: "/department-head/evaluations",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function DepartmentHeadLayout({ children }: { children: React.ReactNode }) {
  ...
  const [deptHeadUser, setDeptHeadUser] = useState<{ full_name: string; role: string; department: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Load user data
    const user = getUser('DEPARTMENT_HEAD');
    if (user) {
      setDeptHeadUser(user);
    }
    setLoading(false);
  }, [router]);
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-3 border-b border-slate-200 px-6 py-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Dept. Head Panel</p>
            </div>
          ...
          <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
            {navigation.map((item) => {
              const isActive = pathname === item.href || (item.href !== "/department-head" && pathname.startsWith(item.href));
              return (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          <div className="border-t border-slate-200 px-4 py-4">
            <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-orange-100 text-orange-700 text-sm font-bold">
                {deptHeadUser?.full_name?.charAt(0)?.toUpperCase() || "D"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{deptHeadUser?.full_name || "Dept. Head"}</p>
                <p className="text-xs text-slate-500">{deptHeadUser?.department || "Department"}</p>
              </div>
