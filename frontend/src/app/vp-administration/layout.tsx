...
const navigation: NavItem[] = [
  ...
  {
    label: "Facilities",
    href: "/vp-administration/facilities",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: "Discipline",
    href: "/vp-administration/discipline",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: "Inventory",
    href: "/vp-administration/inventory",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: "Reports",
    href: "/vp-administration/reports",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function VPAdministrationLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [vpUser, setVpUser] = useState<{ full_name: string; role: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Load user data
    const user = getUser('VP_ADMINISTRATION');
    if (user) {
      setVpUser(user);
    }
    setLoading(false);
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-3 border-b border-slate-200 px-6 py-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">VP Admin Panel</p>
            </div>
          ...
          <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
            {navigation.map((item) => {
              const isActive = pathname === item.href || (item.href !== "/vp-administration" && pathname.startsWith(item.href));
              return (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          <div className="border-t border-slate-200 px-4 py-4">
            <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-purple-100 text-purple-700 text-sm font-bold">
                {vpUser?.full_name?.charAt(0)?.toUpperCase() || "V"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{vpUser?.full_name || "VP Admin"}</p>
                <p className="text-xs text-slate-500">{vpUser?.role || "VP_ADMINISTRATION"}</p>
              </div>
